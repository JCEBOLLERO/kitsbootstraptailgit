    ...
    <div class="row justify-content-center">
        <div class="col-lg-8">
            ...
            <div class="d-flex justify-content-between align-items-center mb-4">
                <div>
                    <h2 class="m-0 mb-1">Editar Artículo</h2>
                    <p class="m-0">Editar un Artículo</p>
                </div>
                <div>
                    ...
                    <form action="{{ route('articulos.update', $articulo->id) }}" method="POST">
                        ...
                        {{-- FAMILIA --}}
                            ...
                            <select name="familia_id" id="familia_id"
                                    class="form-select mb-3 @error('familia_id') is-invalid @enderror">
                                @foreach($familias as $id => $descripcion)
                                    <option value="{{ $id }}"
                                        {{ old('familia_id', $articulo->familia_id) == $id ? 'selected' : '' }}>
                                        {{ $descripcion }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        {{-- PROVEEDOR --}}
                            ...
                            <select name="proveedor_id" id="proveedor_id"
                                    class="form-select mb-3 @error('proveedor_id') is-invalid @enderror">
                                @foreach($proveedores as $id => $Nombre)
                                    <option value="{{ $id }}"
                                        {{ old('proveedor_id', $articulo->proveedor_id) == $id ? 'selected' : '' }}>
                                        {{ $Nombre}}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            value = "{{ old('preciocosto', $articulo->preciocosto) }}"
                            ...
                            value = "{{ old('refprove', $articulo->refprove) }}"
